// A copy of https://github.com/disarticulate/y-webrtc/blob/master/src/y-webrtc.js

import * as ws from 'https://esm.sh/lib0/websocket.js'
import * as map from 'https://esm.sh/lib0/map.js'
import * as error from 'https://esm.sh/lib0/error.js'
import * as random from 'https://esm.sh/lib0/random.js'
import * as encoding from 'https://esm.sh/lib0/encoding.js'
import * as decoding from 'https://esm.sh/lib0/decoding.js'
import { ObservableV2 } from 'https://esm.sh/lib0/observable.js'
import * as logging from 'https://esm.sh/lib0/logging.js'
import * as promise from 'https://esm.sh/lib0/promise.js'
import * as bc from 'https://esm.sh/lib0/broadcastchannel.js'
import * as buffer from 'https://esm.sh/lib0/buffer.js'
...
import * as cryptoutils from './y-webrtc_crypto.js'
...
const validMessage = async (room, data) => {
  ...
  try {
    ...
    console.warn('[Y-WebRTC] InvalidDataFromUpdate', { e, room, data })
    return false
  }
  return valid
}
...
export class Room {
   ...
   * @param {function} validateUpdate
   */
  constructor(doc, provider, name, key, validateUpdate) {
    /**
 ...
 * @return {Room}
 */
const openRoom = (doc, provider, name, key, validateUpdate) => {
  // there must only be one room
  if (rooms.has(name)) {
    throw error.create(`A Yjs Doc connected to room '${name}' already exists!`)
  }
  const room = new Room(doc, provider, name, key, validateUpdate)
...
export class WebrtcProvider extends ObservableV2 {
  ...
  constructor(
    ...
    this.room = null
    this.key.then((key) => {
      this.room = openRoom(doc, this, roomName, key, validateUpdate)
